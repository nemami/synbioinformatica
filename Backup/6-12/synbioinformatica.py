...
def digest(InputDNA, Enzymes):
	...
	for n in range(lastIt):
		...
		print "current enzyme: " + currentEnzyme.compsite
		print "next enzyme: " + nextEnzyme.compsite
		ALL = 0
		ARL = 0
		...
		if abs(TO) < abs(BO) and direction == "sense" or abs(TO) > abs(BO) and direction == "antisense":
			...
			# digested.bottomLeftOverhang = Overhang(reverseComplement(InputDNA.sequence[currentEnd+ALL-difference:currentEnd+ALL]))
		else:
			print "has BLO"
			digested.topLeftOverhang = Overhang('') 
			if currentStart - len(currentEnzyme.alpha_only_site) < 0:
			...
			# digested.topLeftOverhang = Overhang(InputDNA.sequence[currentEnd+ALL-difference:currentEnd+ALL])
			# digested.bottomLeftOverhang = Overhang('')
		if direction == "sense":
			start = nextStart
			TO = NTO
		...
		print "next difference: " + str(difference) 
		if abs(TO) < abs(BO) and direction == "sense" or abs(TO) > abs(BO) and direction == "antisense":
			print "has BRO"
			print difference
			digested.topRightOverhang = Overhang('')
			print digDiff - len(nextEnzyme.alpha_only_site)
			...
			# digested.topRightOverhang = Overhang('')
			# digested.bottomRightOverhang = Overhang(reverseComplement(InputDNA.sequence[currentEnd+ALL:currentEnd+ALL+difference]))
		else:
			print "has TRO"
			digested.topRightOverhang  = Overhang(InputDNA.sequence[nextStart+ARL:nextStart+ARL+difference]) 
			digested.bottomRightOverhang = Overhang('')
			# digested.topRightOverhang = Overhang(InputDNA.sequence[currentEnd+ALL:currentEnd+ALL+difference])
			# digested.bottomRightOverhang = Overhang('')
